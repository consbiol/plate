...
export function buildTerrainFractionsFromTypeCounts({ typeCounts, preGlacierStats, gridWidth, gridHeight, era } = {}) {
  const totalFallback = (Number(gridWidth) > 0 && Number(gridHeight) > 0)
  ...
  const f_land = f_lowland + f_desert + f_highland + f_alpine + f_tundra
    + f_city + f_cultivated + f_polluted + f_bryophyte;
