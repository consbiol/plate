...
function buildLandFeatureMasks(ctx, {
    ...
    const isLandCultivationEra = (ctx.era === '文明時代' || ctx.era === '海棲文明時代');
    const isLandCityEra = (ctx.era === '文明時代');
    ...
    if (isLandCultivationEra || isLandCityEra) {
        for (let gy = 0; gy < ctx.gridHeight; gy++) {
            for (let gx = 0; gx < ctx.gridWidth; gx++) {
                const idx = gy * ctx.gridWidth + gx;
                if (colors[idx] !== lowlandColor) continue;
        ...
        if (isLandCityEra && countPolluted > 0) {
...
function buildSeaFeatureMasks(ctx, {
    ...
    const isSeaCultivationEra = (ctx.era === '文明時代' || ctx.era === '海棲文明時代');
    const isSeaCityEra = (ctx.era === '海棲文明時代');
    ...
    if (isSeaCultivationEra || isSeaCityEra) {
        for (let gy = 0; gy < ctx.gridHeight; gy++) {
            for (let gx = 0; gx < ctx.gridWidth; gx++) {
                const idx = gy * ctx.gridWidth + gx;
        ...
        if (isSeaCityEra && countSeaPolluted > 0) {
            generatePollutedAreas({
                ...clusterInputs,
                derivedRng: resolvedDerivedRng
            }, {
