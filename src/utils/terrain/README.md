...
- **`terrainRunner.js`**: Generate/Drift/Revise の実行ロジックを切り出し（Web Worker化の下地）
- **`centers.js`**
  - `sampleLandCenters(inputs, rng)`: 中心点サンプリング
  - `computeScoresForCenters(inputs, centers, centerParameters)`: スコア計算
  - `computeOwnerCenterIdx(inputs, centers)`: 最寄り中心の割当
- **`landmask.js`**
...
- **`glacierRows.js`**: 平均気温→氷河行数（平滑化の状態は外から注入）
