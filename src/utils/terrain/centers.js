...
const MIN_CENTERS_Y = 1;
const MAX_CENTERS_Y = 10;
const EDGE_MARGIN = 10;
const MAX_SAMPLE_ATTEMPTS = 1000;

const DISTANCE_WARP_AMPLITUDE = 0.03;
...
export function sampleLandCenters(ctx, rng, minDistanceOverride) {
  ...
  const yCenters = Math.max(
    MIN_CENTERS_Y,
    Math.min(MAX_CENTERS_Y, Math.min(gridHeight, centersY))
  );
  ...
  for (let i = 0; i < yCenters; i++) {
    ...
    let attempts = 0;
    let valid = false;
    while (!valid && attempts < MAX_SAMPLE_ATTEMPTS) {
      const cx = Math.floor(random() * gridWidth);
      const cy = Math.floor(random() * gridHeight);
      newCenter = { x: cx, y: cy };
      const isNearEdge = cx < EDGE_MARGIN || cx >= gridWidth - EDGE_MARGIN ||
                         cy < EDGE_MARGIN || cy >= gridHeight - EDGE_MARGIN;
      if (isNearEdge) {
        valid = false;
  ...
  for (let gy = 0; gy < gridHeight; gy++) {
    for (let gx = 0; gx < gridWidth; gx++) {
      ...
      for (let ci = 0; ci < centers.length; ci++) {
        ...
        const fractalN = fractalNoise2D(gx * FRACTAL_NOISE_SCALE, gy * FRACTAL_NOISE_SCALE, 3, 0.5);
        const angularN = noise2D(Math.cos(angle) * 10, Math.sin(angle) * 10);
        const distanceWarp = (fractalN * 0.30 + angularN * 0.70) * DISTANCE_WARP_AMPLITUDE * rMaxPerCenter[ci];
        ...
        const biasTerm = biasStrength > 0 ? biasStrength * Math.exp(-(dn * dn) * BIAS_SHARPNESS) : 0;
